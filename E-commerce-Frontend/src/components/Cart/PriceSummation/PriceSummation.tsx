...
interface PriceSummationPayload {
  totalMRP: number;
  platformFee: number;
}

const PriceSummation = ({ totalMRP, platformFee } : PriceSummationPayload) : JSX.Element => {
  const { cartSize } = useAppSelector((state) => state.cartSlice);
  const { addressObject } = useAppSelector((state) => state.userSlice);

  let totalAmount: number = Number(totalMRP) + Number(platformFee);
