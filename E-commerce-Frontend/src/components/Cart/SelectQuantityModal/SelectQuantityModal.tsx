...
interface SelectQuantityModalPayload {
  selectedCartItem: CartItem | null;
  setSelectedCartItem: (selectedCartItem: CartItem | null) => void;
  setIsSelectQuantityModalOpen: (isSelectQuantityModalOpen : boolean) => void;
  getCart: () => void; 
}

const SelectQuantityModal = ({ selectedCartItem, setSelectedCartItem, setIsSelectQuantityModalOpen, getCart } : SelectQuantityModalPayload) : JSX.Element => {
  const showNotification = useNotification();

  const { cartSize } = useAppSelector((state) => state.cartSlice);
  const dispatch = useAppDispatch();

  const stockSize = selectedCartItem?.stockSize || 0;
  ...
  const [selectedQuantity, setSelectedQuantity] = useState<number>(initialQuantity);
  const [loading, setLoading] = useState<boolean>(false);

  const updateQuantityInCartItem = () : void => {
    setLoading(true);
