...
interface SelectSizeModalPayload {
  selectedCartItem: CartItem | null;
  setSelectedCartItem: (selectedCartItem: CartItem | null) => void;
  setIsSelectSizeModalOpen: (isSelectSizeModalOpen : boolean) => void;
  getCart: () => void; 
}

const SelectSizeModal = ({ selectedCartItem, setSelectedCartItem, setIsSelectSizeModalOpen, getCart } : SelectSizeModalPayload) => {
  const showNotification = useNotification();
   
  const availableSizes = selectedCartItem?.availableSizes || [];

  const [selectedSize, setSelectedSize] = useState<string>(selectedCartItem?.size || "");
  const [loading, setLoading] = useState<boolean>(false);

  const updateSizeInCartItem = () : void => {
    setLoading(true);
