...
interface ProductDetailsModalPayload {
  dbOrderID: string;
  showProductDetails: boolean;
  setShowProductDetails: (showProductDetails : boolean) => void;
  setSelectedOrder: (selectedOrder: null) => void;
}
...
const ProductDetailsModal = ({ dbOrderID, showProductDetails=true, setShowProductDetails, setSelectedOrder } : ProductDetailsModalPayload) : JSX.Element => {
  ...
  const getProductDetails = () : void => {
    ...
    OrderService.getProductDetailsOfOrder(dbOrderID)
      .then((res : { status: number, data: { products: OrderedProduct[] } }) => {
        if (res.status === 200) {
