...
interface ProductCardPayload {
  product: Product;
  handleWishlistItem: (productID: string) => void 
}

const ProductCard = ({ product, handleWishlistItem } : ProductCardPayload) : JSX.Element => {
  const showNotification = useNotification();
  
  const [isHoveringOnCard, setIsHoveringOnCard] = useState<boolean>(false);
  const [isHoveringOnWishlistBtn, setIsHoveringOnWishlistBtn] = useState<boolean>(false);
  const [addToWishlistLoading, setAddToWishlistLoading] = useState<boolean>(false);
  const [wishListBoxHeight, setWishListBoxHeight] = useState<number>(0);
    ...
    >
      <div className="relative flex-1 w-full overflow-hidden">
        ...
        <div
          ...
          onMouseEnter={() : void => setIsHoveringOnWishlistBtn(true)}
          onMouseLeave={() : void => setIsHoveringOnWishlistBtn(false)}
          style={{
