...
interface FetchVariantData {
  OtherColorVariants: OtherVariantDetails[];
...
const Product = () : JSX.Element => {
  ...
  const [otherColorVariants, setOtherColorVariants] = useState<OtherVariantDetails[]>([]);
  ...
  const handleOtherVariantClick = (id : string) : void => {
  ...
  const fetchVariantDetails = () => {
    ...
    ProductService.fetchVariantDetails(userID, productID, variantID)
      .then((res : { status: number, data: FetchVariantData }) => {
        if (res.status == 200) {
          ...
          showNotification("error", "Couldn't fetch Variant Details");
        }
      })
      .catch((error) => {
        console.log(error);
