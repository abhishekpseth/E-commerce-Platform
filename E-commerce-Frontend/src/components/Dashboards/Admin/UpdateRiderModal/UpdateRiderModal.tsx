...
interface UpdateRiderModalPayload {
  dbOrderID: string;
  selectedOrderRiderID: string;
  fetchOrders: () => void;
...
const UpdateRiderModal = ({dbOrderID, selectedOrderRiderID, fetchOrders, setSelectedOrder, setIsRiderModalOpen} : UpdateRiderModalPayload) : JSX.Element => {
  const showNotification = useNotification();
  
  const [selectedRider, setSelectedRider] = useState<string>(selectedOrderRiderID);
  const [loading, setLoading] = useState<boolean>(false);
  const [riders, setRiders] = useState<Rider[]>([]);
  const [fetchRiderLoading, setFetchRiderLoading] = useState<boolean>(false);
  
  const fetchRiders = () : void => {
    setFetchRiderLoading(true);
    RiderService.fetchAll()
      .then((res : { status: number, data: { riders: Rider[] } }) => {
        if(res.status === 200){
